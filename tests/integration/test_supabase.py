...
def test_initial_migration_creates_only_bookmark_schema_contract() -> None:
    migration = next(
...
def test_local_data_api_exposes_bookmark_schema() -> None:
    config = Path(__file__).parents[2] / "supabase" / "config.toml"

    assert 'schemas = ["public", "graphql_public", "bookmark"]' in config.read_text()


def test_local_supabase_project_id_is_unique_to_bookmark_service() -> None:
    config = Path(__file__).parents[2] / "supabase" / "config.toml"

    with config.open("rb") as config_file:
        assert tomllib.load(config_file)["project_id"] == "fastapi-bookmark"
...
def test_data_api_accepts_bookmark_schema_profile(
    credentials: IntegrationCredentials,
) -> None:
    response = httpx.get(
