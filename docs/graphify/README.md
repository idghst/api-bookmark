...
flowchart LR
  ...
  BFF -->|"GraphQL + X-Bookmark-Key"| GQL["api-bookmark /graphql"]
  Mobile["bookmark mobile Expo"] -->|"REST + X-Bookmark-Key"| REST["api-bookmark /api"]
  GQL --> Domain["services bookmarks/folders/sections"]
  REST --> Domain
  Domain --> DB["services/db.py execute"]
  DB --> SB[Supabase]
```
...
flowchart TB
  ...
  subgraph http [HTTP]
    ...
    GQL["/graphql app/graphql/schema.py"]
  end
  subgraph auth [인증]
...
| C3 | GraphQL | `app/graphql/{schema,types,inputs,context,errors}.py` — 허브 `Mutation` |
...
고드 노드: `Settings`(56), `AuthContext`(40), `FakeSupabase`(35), `_client()`(33), `Mutation`(31), `ApiError`(29), `GraphQLContext`, `create_app()`, `execute()`, `resolve()`.

설정·인증·테스트 더블이 허브인 것은 정상이다. 도메인 CRUD를 그 파일들에 더 넣지 않는다. 다음 병목은 `execute()` — 북마크/폴더/섹션 서비스와 REST·GraphQL이 모두 여기를 경유한다.
...
| `affected execute` | 서비스 CRUD + REST 라우트 + `schema.py` | DB 실행이 도메인 허브 |

## 자주 쓰는 질의

```bash
graphify query "how do REST bookmark routes reach Supabase"
graphify explain "Mutation"
