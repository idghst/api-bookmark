# Bookmark Production FastAPI Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Turn `fastapi-bookmark` into a production-ready Vercel FastAPI foundation backed by its isolated Supabase project, Supabase Auth, and the `bookmark` custom schema.

**Architecture:** Vercel loads `app/main.py` as one ASGI Function. FastAPI validates configuration, emits structured request logs, exposes versioned routes, validates bearer tokens through Supabase Auth, and creates a request-scoped asynchronous Supabase client whose Data API queries target `bookmark` and retain the caller JWT for RLS.
...
- The database schema is the fixed string `bookmark`; it is not environment-overridable.
...
- Produces fixed values `app_name="Bookmark API"` and `supabase_schema="bookmark"`.
...
`AsyncClientOptions(schema="bookmark", persist_session=False,
...
- Create via CLI: initial `supabase/migrations/*_init_bookmark.sql`
- Create: `tests/integration/__init__.py`
- Create: `tests/integration/test_supabase.py`

**Interfaces:**
- Produces schema `bookmark`.
...
npx --yes supabase@latest migration new init_bookmark
```
...
`/api/v1/auth/me`, and Data API schema exposure using `Accept-Profile: bookmark`.
...
git commit -m "bookmark 스키마 마이그레이션과 통합 검증 추가"
```
