...
Upgrade `fastapi-bookmark` from a minimal FastAPI scaffold to a production-ready
service foundation. The service deploys to Vercel, uses its own Supabase
project, uses Supabase Auth, and accesses tables in the `bookmark` schema
...
Client Bearer JWT
  ...
  -> bookmark schema through the Data API
...
The initial migration creates `bookmark`, applies required role grants, and
...
- A production Data API probe reaches the `bookmark` schema with expected RLS
